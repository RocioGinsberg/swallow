...
在多模型生态并存的时代，Swallow 系统的架构不能被单一的大模型供应商（Vendor Lock-in）所绑架。在第 6 层（模型接入与路由层），我们引入了区别于普通 API 转发网关的核心组件——**能力协商器 (Capability Negotiator)** 与 **方言适配器 (Dialect Translators)**。
...
*   **适配逻辑**：适配器会自动扫描上层传递的 System Prompt 和规则约束，将其转换包裹进对应的 `<thinking>`, `<scratchpad>`, `<instructions>` 等特定 XML 标签中。这能将 Claude 的长文本遵循能力和减少幻觉的表现提升至最佳状态。
...
## 4. 能力降级与兜底策略 (Graceful Degradation & Fallback)

在离线网络、成本压缩策略或主备切换时，请求可能会被路由到较弱的模型（例如不支持原生 Tool Calling 功能的开源模型）。

### 4.1 ReAct 风格降级转化
当能力协商器发现目标模型不具备原生工具调用（Native Tool Calling / Structured Outputs）能力时：
...
2.  **正则提取校验**：在模型返回结果后，适配器不再依赖 SDK 解析 JSON，而是使用强化的正则表达式从输出文本流中提取意图。
