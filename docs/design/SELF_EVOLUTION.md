...
graph TD
    subgraph "Librarian 主线 — 项目级知识沉淀"
        RM["Raw material / 外部摄入产物 / Task artifacts"]
        WC["Wiki Compiler<br/><i>起草 · 综合 · 引用追溯</i>"]
...
| 主线分支 | 输入 | 处理者 | 输出 | 写入路径 |
|---|---|---|---|---|
| **Wiki Compiler**(起草侧) | raw_material、外部摄入产物、已有 wiki/canonical(查重) | Wiki Compiler(specialist) | staged candidates(含 source pack、rationale、conflict flags) | Wiki Compiler 写 staged;canonical 写入仅经 `apply_proposal` |
| **Librarian**(守门侧) | task artifacts、event truth、handoffs、Wiki Compiler 产出的 staged | Librarian(specialist) | staged candidates(冲突检测 / 去重 / supersede 标记完成) | Librarian 写 staged;canonical 写入仅经 `apply_proposal` |
...
```
[A] 起草侧:Wiki Compiler 主线(4 种 mode)
  Mode 1 — draft:
    raw_material → Wiki Compiler 起草新 wiki entry → know_staged
    relation_metadata = (无 / derived_from)
  Mode 2 — refine + supersede:
    raw_material + 旧 wiki_id → Wiki Compiler 起草替代版本 → know_staged
    relation_metadata = supersedes(<wiki_id>)
  Mode 3 — refine + refines:
    raw_material + 旧 wiki_id → Wiki Compiler 起草精化版本 → know_staged
    relation_metadata = refines(<wiki_id>)
  Mode 4 — refresh-evidence(不走 LLM):
    源文件 content_hash 变化 → 仅刷 evidence 锚点(parser_version + span + content_hash)
    不创建新 wiki;know_change_log 记 evidence_refreshed

[B] 守门侧:Librarian 主线
  任务执行 → 产生 task truth / event truth / artifacts / handoffs
   → Librarian 读取显式材料 + Wiki Compiler 产出的 staged
   → 提炼 reusable evidence、staged candidates、dedupe-supersede signals
   → 冲突检测:若 Wiki Compiler 草稿与已有 wiki 矛盾,标 conflict_flag = contradicts(<wiki_id>)
   → 生成结构化变更记录(写入 know_change_log)
   → 写入 / 更新 know_staged

[共同收口]
  know_staged
   → Operator 通过 CLI 或 Web Control Center review:
       - draft / refines / refresh-evidence 草稿:可直接 promote
       - supersede 草稿:Operator 确认旧 wiki 失效后 promote
       - contradicts 草稿:人工二选一(supersede 旧 wiki / reject 新草稿 / 触发更深 audit);系统永远不自动 supersede(P7 / P8)
   → apply_proposal 写入 know_canonical(若 promote)
       - draft → 新 wiki / canonical 入库
       - refine + supersede → 新 wiki 入库,旧 wiki 状态置 superseded(append-only,不删除)
       - refine + refines → 新 wiki 独立入库,refines 关系记入 relations 表
       - refresh-evidence → 仅更新 know_evidence(不经 apply_proposal,因为非 canonical mutation)
```

**Wiki Compiler 与 Librarian 的关系**:Wiki Compiler 是**上游创作者**(从 raw_material 起草,可显式声明 supersede / refines 关系),Librarian 是**下游守门人**(冲突检测 / 去重 / supersede signal 二次验证)。两个 agent 都只能写 staged,canonical 写入仍由 `apply_proposal` 唯一入口完成。让一个 agent 同时做创作 + 守门会失去独立 verdict 的价值,故拆为两个 specialist。

**Evidence Rebuild 边界**:`refresh-evidence` 是技术性刷新,不是 canonical mutation,**不**经 `apply_proposal`,但必须更新 `parser_version`(parser 升级时)以避免 `KNOWLEDGE.md §A "Unversioned Evidence Rebuild"` 反模式 —— 静默 hash 刷新会让旧/新 evidence 不可区分,supersede / verification 判断失效。
