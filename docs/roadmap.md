...
D5 Adapter Discipline + D4 Phase A + LTO-6 Knowledge Plane Facade Solidification + LTO-1 Wiki Compiler 第一阶段 + Hygiene Bundle + LTO-1 Wiki Compiler 第二阶段 + LTO-2 Retrieval Quality / Evidence Serving + LTO-4 Test Architecture 全部已落地(2026-05-03 → 2026-05-04;commits `d67c2ad` `7450953` `883e2a9` `349efa9` `e656bd3` `21f8dc8` `03744f0` `ac2d3ff`,tag `v1.8.0` at `d6f2442`;Hygiene Bundle / LTO-1 stage 2 / LTO-2 / LTO-4 不 cut tag)。

**当前近期队列 = R-entry Real Usage runbook(非开发 phase)**:

- **入口**:`docs/plans/r-entry-real-usage/plan.md`
- **范围**:用 Swallow 自身设计文档材料深测 CLI task / knowledge ingestion / Wiki Compiler dry-run + real draft / operator promote-reject / retrieval-artifacts / Web UI / Tailscale + nginx 展示链路。
- **流程**:不走 plan audit / review / closeout;执行 runbook,记录 `$BASE/notes/r-entry-issues.md`;完成后由 issue log 进入下一 Direction Gate。
- **不做**:不做 Dockerfile、不把 `swl serve` 绑定到非 loopback、不引入认证/多用户/公网语义、不直接把发现转成代码修改。
- **判定**:成功标准不是“没有问题”,而是能把真实问题定位到 CLI / UI / nginx / knowledge / wiki / retrieval / docs 等 surface,并为下一轮选择提供实证信号。

**Direction Gate 候选(R-entry issue log 出现后由 Human 选定)**:
