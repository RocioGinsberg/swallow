...
| **检索基础设施** | Retrieval U-T-Y 已落地:dedicated rerank boundary、retrieval trace、source policy warnings、EvidencePack compatibility view、RawMaterialStore-backed source pointer resolution、summary route boundary 均已进入 baseline |
...
| **主要未完成面** | 深度架构重组 program、TDD/test architecture、Knowledge Plane facade、interface/application boundary、Provider Router / Orchestration / Surface / Governance subtracks、LLM Wiki Compiler、Planner / DAG / Strategy Router 一等化 |
...
| **EvidencePack / Evidence Resolution** | KNOWLEDGE / DATA_MODEL | U-T-Y 已提供 EvidencePack compatibility view 与 source pointer resolution;后续缺口转为 bounded excerpt / conflict / wiki compiler integration | 已完成第一阶段;后续并入 S 或 AD retrieval-serving subtrack |
| **RAG 可观测性 / 质量评估** | KNOWLEDGE / HARNESS | U-T-Y 已补 retrieval mode / fallback / rerank / source policy / regression probes;后续质量评估可在 S 或 eval hardening 中继续 | 已完成第一阶段;不再作为当前主重构阻塞项 |
...
| **深度架构重组 program 缺位** | INVARIANTS / ARCHITECTURE / INTERACTION / engineering docs | AC 提供 pattern 语言,但更彻底的 interface/application/domain/persistence/orchestration/provider/knowledge/governance 重组需要 program 级拆解、顺序、回滚点和验收 | **候选 AD**:Architecture Recomposition / Deep Refactor Program;以 AC/AA 为前置,把 AB/V/W/X/Y/Z 组织为 subtracks,不做单 phase big-bang rewrite |
...
| 已完成实施 | 候选 U/T/Y bundle | Retrieval Observability / EvidencePack / Summary Surface | Retrieval Quality / Knowledge / Interaction | 已在 Retrieval U-T-Y 分支落地并 merge;后续只保留增量 eval / excerpt / UX follow-up |
| 当前推荐下一步 | 候选 AD | Architecture Recomposition / Deep Refactor Program | Architecture / Engineering | 新增 program 级候选;先做 AD0 计划与 AA/V/AB pilot 顺序,再按 subtrack 推进 |
| 推荐次序 2 | 候选 AA | Test Architecture / TDD Harness | Engineering Quality | AD 的测试前置地基;先建立 helpers、CLI runner、分层目录,再拆 `test_cli.py` |
| 推荐次序 3 | 候选 V | Knowledge Plane API Simplification | Knowledge / Architecture | AD 的低风险首个 facade subtrack;收口 public import 面 |
| 推荐次序 4 | 候选 AB | Interface / Application Boundary Clarification | Architecture / Interaction | 作为 AD subtrack 先做 narrow command/query pilot,不全量重组 |
| 推荐次序 5 | 候选 W | Provider Router API Split | Provider Router / LLM Gateway | AD subtrack;Path A/C 调用增长或 router 维护摩擦触发 |
| 推荐次序 6 | 候选 X | Orchestration Facade Decomposition | Orchestration | AD subtrack;准备 D 或主链路维护摩擦升高时推进 |
| 推荐次序 7 | 候选 Y | Surface Command / Meta Optimizer Module Split | Interaction / Self Evolution | AD subtrack;U-T-Y 已完成窄 summary boundary,后续只做无行为变化模块化 |
| 推荐次序 8 | 候选 Z | Governance Apply Handler Split | Truth Governance | AD subtrack;保持 `apply_proposal` 唯一入口,最后推进 |
| 推荐次序 9 | 候选 S | LLM Wiki Compiler / Wiki Refinement Workflow | Knowledge / LLM | AD 地基稳定后再做 raw material → staged wiki/canonical draft |
| 推荐次序 10 | 候选 D | 编排增强(Planner / DAG / Strategy Router) | Orchestration | 后置;等 X 或真实编排瓶颈推动 |
...
- **关键结论**:canonical reuse 可用;P2b/P3b 证明 `.env` 加载正确时 `api_embedding + sqlite_vec + rerank` 能把目标 canonical truth 排到顶部。
- **分流结果**:这些缺口已由 Retrieval U-T-Y 第一阶段吸收;后续只保留 eval / excerpt / UX 增量。
- **后续位置**:R 不再驱动当前主线;当前主线转为 AD architecture recomposition。
...
- **第一阶段状态**:Retrieval U-T-Y 已落地 EvidencePack-compatible serving view、source pointers、RawMaterialStore-backed pointer resolution 与报告可见性。
- **后续增量**:bounded excerpt、conflict flags、artifact evidence span、wiki compiler integration。
- **边界**:EvidencePack 不内嵌 raw bytes 为 Truth;只承载 reference 与必要 excerpt,不改变 source type 语义。
- **排序**:第一阶段完成;后续增量并入 S 或 AD retrieval-serving subtrack。
...
- **第一阶段状态**:Retrieval U-T-Y 已落地 dedicated rerank boundary、retrieval trace/report header、source policy labels/warnings、P1/P2/P3 regression probes 与 staged intake hygiene warnings。
- **后续增量**:persistent embedding index 设计评估、query rewrite、eval hardening、report UX polish。
- **边界**:向量索引仍是辅助召回,不反向定义 Knowledge Truth;query rewrite 不得绕过 source policy 与治理过滤。
- **风险**:中低;第一阶段已避免 schema-heavy 或 platform-heavy 改动,后续仍应保持 source policy 和 Truth before retrieval 边界。
...
- **排序**:U/T/Y narrow 已完成;AC 的 pattern language 现在作为 AD program 的工程锚点。
- **风险**:中;主要风险是过度抽象和 import churn,必须 facade-first、测试先行、小步提交。

### 候选 AD:Architecture Recomposition / Deep Refactor Program

- **核心价值**:把更彻底的系统重组从单次大重构改为 program:先定义 facade / command-query / repository / lifecycle / adapter 边界,再按 AB/V/W/X/Y/Z subtracks 小步执行。
- **工程锚点**:`docs/plans/architecture-recomposition/plan.md`、`docs/engineering/GOF_PATTERN_ALIGNMENT.md`、`docs/engineering/CODE_ORGANIZATION.md`、`docs/engineering/TEST_ARCHITECTURE.md`。
- **前置条件**:AC pattern language 已固定;AA 至少完成 helpers + CLI runner + guard 分层最小地基;每个 subtrack 都必须有 focused tests + full pytest gate。
- **program subtracks**:AD0 program plan / AD1 test harness foundation(AA) / AD2 Knowledge Plane facade(V) / AD3 application command-query pilot(AB) / AD4 Provider Router split(W) / AD5 Orchestration lifecycle decomposition(X) / AD6 Surface commands split(Y) / AD7 Governance private handler split(Z)。
- **边界**:不改变 `docs/design/INVARIANTS.md`;不移动 Control Plane authority;不改变 `apply_proposal` 唯一入口;不把 SQLite local truth 拆成外部服务;不一次性重写 CLI、Orchestrator、Provider Router、Knowledge Plane。
- **风险**:高;必须 facade-first、test-first、小 milestone commits,每个 subtrack 可回滚。
...
- **第一阶段状态**:Retrieval U-T-Y 已澄清 summary route boundary,`--route-mode summary` 明确是 local execution summary,不是 semantic QA answer。
- **后续增量**:`surface_tools/commands/{task,knowledge,route,proposal,reports,doctor}.py` / meta optimizer model、snapshot、proposal builder、review、apply、agent 模块化 / route weight proposal JSON artifact / durable proposal artifact lifecycle / parser-dispatch alignment。
...
**R(done) → U/T/Y(done) → AD0 → AA → V → AB pilot → W/X/Y/Z subtracks → S → D**

排序依据:

1. **U/T/Y 已完成第一阶段闭环**:retrieval observability、EvidencePack/source pointer、summary boundary 已进入 baseline。
2. **AD0 先行**:更彻底重构必须先成为 program plan,明确 subtrack、顺序、测试前置、回滚点。
3. **AA/V 作为首批地基**:AA 降低后续大重构测试成本;V 是低风险 facade-first 真实重构面。
4. **AB 先 pilot**:只抽一个 command/query 样板,不全量重组 CLI/FastAPI/persistence。
5. **W/X/Y/Z 按触碰面进入 AD**:router、orchestration、surface、governance 都是 subtracks,不再各自发散。
6. **S 后置**:等 architecture/test/knowledge facade 地基稳定后再做 LLM Wiki Compiler。
7. **D 最后**:Planner / DAG / Strategy Router 依赖 X 的主链路整理,仍是高回滚成本增强。
...
| 维度 | 当前现状 | 下一步候选 |
|------|----------|------------|
| 知识治理 | RawMaterialStore 边界已落地;Wiki / Canonical 默认主入口稳定;Candidate R 证明 canonical reuse 可用;EvidencePack 第一阶段已落地;LLM Wiki Compiler 仍是实现缺口 | AD 地基后接 S |
| 检索质量 | U-T-Y 已补 retrieval trace、dedicated rerank、source policy warning、EvidencePack、source pointers 与 summary boundary;后续是 eval/excerpt 增量 | S 或后续 eval hardening |
...
| 系统设计重构 | GoF-style 职责语言已形成;更彻底重组需要 program 级执行计划而非单 phase 大搬家 | 候选 AD;参考 `docs/plans/architecture-recomposition/plan.md` |
| 代码组织 | runtime code 已按 `truth_governance/`、`orchestration/`、`provider_router/`、`knowledge_retrieval/`、`surface_tools/` 分层,但 knowledge plane / router / orchestrator / CLI 仍有 API 面和职责边界重组空间 | AD subtracks:AA/V/AB/W/X/Y/Z |
