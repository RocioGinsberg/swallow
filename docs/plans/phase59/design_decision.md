...
Phase 59 拆为 3 个 slice：S1 route 注册 + alias 迁移（最低风险 3 分）、S2 executor 配置 + dispatch（低风险 4 分）、S3 doctor 探针扩展（最低风险 3 分）。S2 依赖 S1（新 route 需先注册），S3 独立。实施时必须同步移除 `ROUTE_NAME_ALIASES` 中 `local-codex` 条目、新增 `EXECUTOR_ALIASES["codex"]`，并清理剩余 `local-cline` alias，避免继续制造误导。
...
1. **移除 alias**：
   - `ROUTE_NAME_ALIASES` 从 `{"local-codex": "local-aider", "local-cline": "local-claude-code"}` 改为 `{}` 
...
3. **不恢复 `local-cline` 兼容** — Phase 59 之后不再保留该误导性 alias
