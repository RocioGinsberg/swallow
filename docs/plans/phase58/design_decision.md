...
Phase 58 拆为 3 个 slice：S1 `swl note` 灵感捕获（低风险 3 分）、S2 clipboard transport + `generic_chat_json`（低风险 5 分）、S3 staged review 可见性（最低风险 3 分）。S1/S2 无顺序依赖可并行，S3 依赖 S1 新增的 `topic` 字段。实施时必须保留 `staged-*` candidate_id、将 omitted `--format` 作为 auto-detect、为 clipboard path 提供非 Path 的 source_ref override、增加受限 flat message-list JSON parser，并确保 topic 在 inspect / update 路径不丢失。
...
**目标**：把 clipboard 作为现有 `swl ingest <source_path>` 的低摩擦输入载体，并支持其他 chatbot 的受限 flat message-list JSON。S2 不替代文件路径摄入，不引入 URL/shared-link 抓取，不创建新的知识存储通道。

**影响范围**：
- `src/swallow/cli.py` — `swl ingest` 子命令参数调整 + clipboard 分支逻辑
- `src/swallow/ingestion/parsers.py` — 新增 `generic_chat_json` 格式 parser 与测试
- `src/swallow/ingestion/pipeline.py` — 新增 bytes/clipboard helper 或 source_ref override
- 不新增 Python 依赖
...
1. **CLI 参数调整**：
   ...
   - 新增 `--format` 参数（choices：`chatgpt_json|claude_json|open_webui_json|generic_chat_json|markdown`，默认 `None`）
...
2. **`generic_chat_json` parser**：
   - `SUPPORTED_INGESTION_FORMATS` 增加 `generic_chat_json`
   - 支持 `{ "messages": [...] }` 与 `[ ... ]` 两类 flat message-list JSON
   - 每条 message 支持 role-like 字段：`role` / `sender` / `from` / `author`
   - 每条 message 支持 content-like 字段：`content` / `text` / `message`
   - content 支持 string、string list、OpenAI-style text parts（如 `[{ "type": "text", "text": "..." }]`）
   - role 归一化保持保守：`human|me` → `user`，`ai|bot|assistant` → `assistant`，`system` 保留；无法识别 role 或 content 的 item 跳过
   - 不恢复 ChatGPT `mapping` 树、不处理 provider-specific branch semantics；ChatGPT 官方导出仍走 `chatgpt_json`
   - auto-detect 只处理无歧义 flat array；`{"messages": [...]}` 建议显式 `--format generic_chat_json`，避免与 Open WebUI parser 语义冲突

3. **剪贴板读取函数 `_read_clipboard() -> bytes`**：
   - 按平台选择 subprocess 命令：
...
**风险评级**：
- 影响范围: 2（cli.py + parsers.py + pipeline helper，但仍不改 staged/retrieval truth）
...
- `swl ingest chat.json --format generic_chat_json` 可解析 flat message-list JSON
- `swl ingest --from-clipboard --format generic_chat_json` 可解析剪贴板中的 flat message-list JSON
- `swl ingest`（无参数）报错提示
- pytest 覆盖 clipboard 路径（mock subprocess）、格式探测路径、互斥校验、`generic_chat_json` 的 `{messages:[...]}` / `[{...}]` 两类结构
...
S2 (clipboard transport + generic_chat_json) — 独立，无前置依赖
S3 (review visibility) — 依赖 S1 的 topic 字段定义
```

**推荐实施顺序**：S1 → S3 → S2

S1 先落地（定义 `topic` 字段 + `operator_note` source_kind），S3 紧随（展示新字段），S2 最后（独立的 CLI / parser 入口扩展）。
...
5. **不做 URL / shared link 摄入** — 不抓取 ChatGPT shared link、网页分享链接或任何需要登录态的 remote URL
6. **不引入复杂 provider adapter/plugin 抽象** — `generic_chat_json` 仅覆盖受限 flat message-list JSON
7. **不修改 ingestion filter 语义** — 现有 `filter_conversation_turns()` 不扩展为新的质量策略
8. **不新增 `StagedCandidate.stage` 字段** — `status` 字段（pending/promoted/rejected）已承担此语义
