...
Phase 58 (A-lite) 将"灵感捕获 / 外部讨论回收 / staged review 可见性"三个低摩擦入口打通到 staged knowledge 管线，让系统从"任务执行工具"升级为"思考-执行一体工具"。完整 Brainstorm topology 后置。核心原则：不引入新的知识存储通道，所有入口共享 staged → review → promote/reject 管线。
...
2. **外部讨论回收**：Open WebUI / ChatGPT / Claude 中完成的 brainstorm，结论回流需要导出 JSON → 放到本地路径 → `swl ingest <path>`。每次导出落盘是无意义摩擦
...
2. **G2 — `swl ingest --from-clipboard` 剪贴板摄入**：从剪贴板读取外部讨论导出，走现有 ingestion pipeline
...
- **新增解析器格式**：Open WebUI / ChatGPT / Claude / Markdown 解析器已全部存在，本轮不扩展
...
### S2: `swl ingest --from-clipboard [--format open_webui_json|markdown]`
...
3. **review 可见性提升**：stage list、stage inspect、task staged 三个视图均展示 `source_kind`、`source_ref`、`topic`
4. **现有行为无回归**：`swl ingest <path>` 所有现有用法保持不变
5. **测试覆盖**：`swl note` 写入 / topic 字段 / clipboard ingest / report 展示字段均有 pytest 覆盖
...
| S2 (clipboard ingest) | 否 | 复用现有 ingestion pipeline，格式解析已有 eval 基线 |
