...
- 使用 `httpx.post()` 直接调用 API（方案 B），endpoint / API key 复用现有 env vars（`AIWF_NEW_API_BASE_URL` / `AIWF_NEW_API_KEY`）
...
- 环境变量：`AIWF_NEW_API_BASE_URL` / `AIWF_NEW_API_KEY`（Agent LLM 调用复用现有配置）
