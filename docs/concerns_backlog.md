...
|-------|-------|---------|---------|
| 36 | S1: Librarian 持久化 | `_apply_librarian_side_effects()` 中 save_state → index 重建为顺序执行，中间步骤失败会导致 state 与 index 不一致 | 引入事务性 checkpoint 或 WAL 机制时 |
| 38 | S2: fallback 成本统计 | `EVENT_TASK_EXECUTION_FALLBACK` 的 `token_cost` 未计入 Meta-Optimizer route stats 的 `total_cost` / `cost_samples`，fallback 执行成本被遗漏 | 下次触碰 meta_optimizer 成本逻辑时，将 fallback token_cost 累加到 fallback route stats |
| 40 | S2+S3: debate loop 代码重复 | `_run_single_task_with_debate()` 与 `_run_subtask_debate_retries()` 核心循环结构近似重复（~170 行），修改 debate 策略需同步两处 | 下次触碰 debate/review 逻辑时评估提取共享 `_debate_loop_core()` |
| 40 | S2: debate retry telemetry | debate 轮次的 executor 事件仍以 `executor.completed` 记录，Meta-Optimizer route health 聚合会将 retry 与正常执行混在一起，膨胀请求计数和失败率 | 下次触碰 meta_optimizer route health 逻辑时，排除带 `review_feedback` 的事件或用独立 event_type 标记 |
