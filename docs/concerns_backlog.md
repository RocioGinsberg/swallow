...
| 66 | high audit finding: dead `run_consensus_review(...)` wrapper | Phase 66 Block 2 found `src/swallow/review_gate.py:617` has no production or test callsite; async consensus and `run_review_gate(...)` are the live paths | Small hygiene cleanup phase can remove it or intentionally wire it as public API |
