...
最近一次 triage：2026-05-04(LTO-2 Source Scoping review)。新增 1 项 Active Open(`LTO-2 Source Scoping review follow-ups`,聚合 review 中 3 个 [CONCERN])。前几轮 triage:2026-05-04(post-LTO-2 merge),2026-05-04(Hygiene Bundle),2026-05-04(post-v1.8.0 + LTO-1 closeout)。
...
| Orchestration Lifecycle Decomposition (LTO-8) follow-up | `execution_attempts.debate_loop_core` and `debate_loop_core_async` accept 9 callable parameters including `record_round` / `record_exhausted` / `persist_exhausted` / `apply_feedback` / `store_feedback`. The helper holds loop termination control (DEBATE_MAX_ROUNDS=3) and decides *when* the orchestrator-injected callables fire. Callables are append-only telemetry not state-advancement, so `INVARIANTS.md "Control only in Orchestrator / Operator"` is technically respected, but the structural pattern is the close cousin of the `save_state` closure-injection prohibition the audit added in C-2. Future readers may misread this as license for helpers to hold control of state-touching callbacks. | Design follow-up: either (a) split debate loop into pure orchestrator-side controller plus helper-side next-state computer, or (b) document explicitly that `record_*` callables in `debate_loop_core` must be telemetry-only and the loop's termination decision is mechanical. Defer to LTO-11 (Planner / DAG / Strategy Router) where loop control patterns are first-class。原 (2) "orchestrator.py reduction conservative" 已在 LTO-8 Step 2 完成 + roadmap 簇 C 归档反映,移到 Resolved。 | Revisit at LTO-11 |
| LTO-1 Stage 2 evidence schema reuse risk | Wiki Compiler 第二阶段 plan 明确复用现有 task-scoped `knowledge_evidence` store 来承载 promotion-derived evidence support objects,不创建 DATA_MODEL §3.3 standalone `know_evidence` 表。LTO-2 已在不做 schema migration 的前提下加入 semantic global evidence identity(`evidence-src-<source_anchor_key>`),但 design/runtime 命名与 physical `know_evidence` 表仍未对齐。 | Accepted as a compatibility decision;不阻塞当前 main。任何后续要引入 standalone `know_evidence` physical table、object-storage-backed evidence truth、evidence lookup index/backfill、或 evidence schema migration 的 phase,必须重新审视 `know_evidence` vs `knowledge_evidence` 并更新 DATA_MODEL / migration 策略。 | Before any evidence schema migration, object-storage evidence backend, evidence lookup index/backfill, or physical `know_evidence` table work |
| LTO-13 long-running write routes(R2-1 deferred) | `POST /api/tasks/{id}/run|retry|resume|rerun` 当前是 minute-scale 阻塞路由,plan §Sync vs async 选择了 accept-long-request contract;UI 用 `pendingTaskAction` 防重复提交,但缺真实 fire-and-poll / background runner。 | Non-blocking;deferred follow-up,等真实多用户 / 多并发 / browser timeout 痛点出现再开 phase。 | Real concurrency / browser timeout pain in Web Control Center triggers a `feat/web-fire-and-poll` phase |
| LTO-13 deferred operator UX | staged-knowledge `--force` Web UX(R2-4 决策为 Web 不暴露 force);文件上传管线(`.swl/uploads/` lifecycle);route policy admin write controls。 | Non-blocking;每项独立设计 surface,等真实需求触发。 | Operator 在 Web 上反复需要 force / 上传外部文件 / 调 route policy 时再开各自 phase |
| LTO-2 Source Scoping review follow-ups | (C1) `DECLARED_DOCUMENT_PRIORITY_BONUS = 1000` / `SOURCE_POLICY_NOISE_PENALTY = 250` 是无注释的硬编码数,priority 大小压过 rerank 与词法分;(C2) `truth_reuse_visibility.task_knowledge.reason_counts` 的四个分类来自三个独立聚合,可能重复计数,加和不等于 `skipped_count`;(C3) `canonical_registry.reason_counts` 仅暴露 `query_no_match`,`policy_excluded` / `status_not_active` 没有出现在 visibility 报告。 | 不阻塞 LTO-2 merge:实现满足 plan + 通过 audit 5 concerns;但 score 调优、reason-count 语义、canonical 侧 reason 覆盖应在后续 retrieval quality 调优 phase 中处理。 | 真实使用中出现 (a) operator 反馈优先级过强压过相关性 / (b) report 数字解读不一致 / (c) canonical 被静默过滤 时,开 retrieval policy 调优 slice。 |
...
|-------|---------|-----------|---------|
| LTO-1 Stage 2 | Source-pack evidence idempotency scoped to `(candidate_id, source_pack_index)`, so different staged candidates citing the same source anchor could create duplicate evidence support objects | LTO-2 Retrieval Quality / Evidence Serving | Merge `03744f0`: added `source-anchor-v1` identity over `source_ref/content_hash/parser_version/span/heading_path`, stable `evidence-src-<source_anchor_key>` ids, cross-candidate reuse via `knowledge_object_exists(base_dir, evidence_id)`, deterministic `derived-from-v1` relation ids, retrieval metadata propagation, EvidencePack/source pointer dedup, and `tests/eval/test_lto2_retrieval_quality.py` eval coverage |
| LTO-7 | Provider Router Split follow-up (2)/(3): `router.py` 伸入 `route_policy.py` / `route_registry.py` 私有名,且 `_BUILTIN_ROUTE_FALLBACKS` 由 router 持有 | Hygiene Bundle | `route_policy.py` / `route_registry.py` 增加公共 payload/normalizer/fallback baseline 入口;`router.py` 改走公共接口并删除 route_selection 私名 wrapper;`_BUILTIN_ROUTE_FALLBACKS` 搬到 `route_registry.py` 由 registry owner 维护 |
| LTO-6 | LTO-6 PR review C1: `knowledge_plane.py` 中 paired `render_*` / `build_*` report aliases 扩大 public facade surface | Hygiene Bundle | 删除 facade 中冗余 `build_*_report` / `build_*_summary` report wrappers;保留 `render_*` 用于报告渲染,保留真实 `build_*` 仅用于对象/summary 构造;CLI 调用点同步改用 `render_*` |
| LTO-1 | LTO-1 PR review C1: `WikiCompilerExecutor` empty subclass(`wiki_compiler.py:492-493`)与现有 4 个 specialist 单类模式不一致,docstring 说 "compatibility wrapper" 但无任何兼容对象 | LTO-1 review-fix | commit `32a5cbb refactor(wiki): remove executor compatibility wrapper`:删除 `WikiCompilerExecutor` 空 subclass,EXECUTOR_REGISTRY 改注册 `WikiCompilerAgent` 直接;~5 行 cleanup,与 LTO-13 / LTO-6 同类 review concern 处理方式一致 |
| LTO-13 | LTO-13 PR review C1: `surface_tools/web/schemas.py:12` 直接 import `swallow.knowledge_retrieval.staged_knowledge.StagedCandidate`,违反 Direction 决定中"避免直接 reach `knowledge_retrieval.*`"约束;放大 `ARCHITECTURE_DECISIONS.md §3.1 D1` 偏离 | LTO-13 review-fix(merge 前) | `application/commands/knowledge.py` 公共导出 `StagedCandidate`,`schemas.py:12` 改为从 application 层 import;3 行修复;为 LTO-6 后续 facade migration 提供干净的边界 |
| LTO-13 | LTO-13 PR review N1: `_static_dir()` 用 `Path.cwd()` 作为 `resolve_path` 的 base 是 dead-arg(`__file__` 已 absolute) | LTO-13 review-fix(merge 前) | `_static_dir()` 改为 `Path(__file__).parent / "static"`;避免 `Path.cwd()` dead base 且不触发 `Path.resolve()` invariant guard |
| LTO-9 | LTO-9 Step 1 intentionally deferred broad task / knowledge / artifact CLI command-family migration and the remaining route command adapters | LTO-9 Step 2 / LTO-13 / LTO-1 | `adapters/cli_commands/` 现有 9 个 command adapter(`audit / knowledge / meta_optimizer / proposals / route_metadata / route / synthesis / tasks / wiki`),完整覆盖原 deferred 范围。LTO-9 Step 2 完成 task/knowledge/proposal 主体,LTO-13 加 staged-knowledge / proposal write,LTO-1 加 wiki adapter |
| LTO-8 | LTO-8 Step 1 reduction 保守(3853 → 3331,~14%);roadmap 应标 "Step 1 done not fully done" | LTO-8 Step 2 / 簇 C 归档 | LTO-8 Step 2 完成 `harness.py` 2077 → 1032 (-50%) + 9 helper 模块化 + helper-side append_event allowlist;v1.6.0 标 cluster C closure;roadmap 现标"簇 C 已归档"形态,LTO-8 详细 step 历史归 git log |
| LTO-7 | `test_route_metadata_writes_only_via_apply_proposal` allowlist named `provider_router/router.py` only, while actual `save_route_*` writers lived in `route_metadata_store.py` | LTO-9 Step 1 M4 | `truth_governance/truth/route.py` now imports physical route metadata writers directly from `provider_router/route_metadata_store.py`; invariant guards name `route_metadata_store.py` as physical writer owner and document `provider_router/router.py` only as a legacy compatibility facade exception |
