...
| Meta Docs Sync | Roadmap audit (closeout) | INVARIANTS §0 第 4 条以及 ARCHITECTURE / STATE_AND_TRUTH / EXECUTOR_REGISTRY / INTERACTION 等 7+ 处把 `apply_proposal()` 定义为 canonical / route / policy 写入唯一入口，§9 守卫测试集列出 `test_canonical_write_only_via_apply_proposal` / `test_only_apply_proposal_calls_private_writers` / `test_route_metadata_writes_only_via_apply_proposal` 三条，但 `grep -rn "apply_proposal" src/ tests/` 零匹配，实际 canonical / route / policy 写入由 `swl knowledge apply-suggestions` / `swl route apply-proposals` 等 CLI 子命令直接调底层 store——设计文档与代码出现宪法级漂移 | 需专项 phase 决议；推荐方案 (a) 实现 `apply_proposal()` 函数把现有 CLI 写路径收敛进去并补齐守卫测试；备选 (b) 反向修订 INVARIANTS 承认 CLI 等价入口。INVARIANTS 是“只增不改”，倾向 (a)。本条由 Meta Docs Sync 收尾的 cross-doc 审计发现，不来自单一 phase review |
