...
最近一次 triage：2026-05-04(Hygiene Bundle)。结论:LTO-6 facade alias cleanup 与 LTO-7 provider-router private-name / fallback ownership follow-up 已在 Hygiene Bundle 消化,移到 Resolved。无新 blocker。Active Open 保留:R-observation quality edge cases / LTO-8 debate_loop_core callable / LTO-13 long-running write routes / LTO-13 deferred operator UX。前两轮 triage:2026-05-04(post-v1.8.0 + LTO-1 closeout),2026-05-01(R-entry / backlog hygiene)。
...
| LTO-1 Stage 2 evidence schema reuse risk | Wiki Compiler 第二阶段 plan 明确复用现有 task-scoped `knowledge_evidence` store 来承载 promotion-derived evidence support objects,不创建 DATA_MODEL §3.3 standalone `know_evidence` 表。该决策避免本 phase 变成 schema migration,但留下 design/runtime 命名与 global evidence-id 语义不完全对齐的长期风险。 | Accepted for LTO-1 Stage 2 Plan Gate as a compatibility decision;不阻塞本 phase。任何后续要引入 global evidence ids、object-storage-backed evidence truth、或 evidence schema migration 的 phase,必须重新审视 `know_evidence` vs `knowledge_evidence` 并更新 DATA_MODEL / migration 策略。 | Before any evidence schema migration, object-storage evidence backend, or global evidence-id feature |
...
| LTO-2 Retrieval Quality / Evidence Serving | LTO-1 Stage 2 deliberately scopes source-pack evidence idempotency to `(candidate_id, source_pack_index)`,so different staged candidates that cite the same `source_ref` / `content_hash` / `parser_version` / `span` may create duplicate evidence support objects. | 在 LTO-2 evidence/retrieval quality 增量中统一设计 source-anchor dedup、global evidence identity、relation expansion 去重和 eval coverage;不在 LTO-1 Stage 2 中提前做 cross-candidate dedup |
