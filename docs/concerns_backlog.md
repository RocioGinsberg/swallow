...
| Knowledge Plane facade naming cleanup (LTO-6) | LTO-6 PR review C1: paired `render_*` / `build_*` aliases in `knowledge_plane.py` inflate the public facade surface after caller migration has completed. | Non-blocking; merge LTO-6 as-is, then prefer a tiny follow-up that keeps `render_*` for report rendering and `build_*` for object construction, deleting redundant aliases before Wiki Compiler expands callers. | Before or at the start of Wiki Compiler first-stage implementation |
| Provider Router Split (LTO-7) follow-up | (2) `router.py` reaches into `_`-prefixed names in `route_policy.py` / `route_registry.py` (`_apply_route_policy_payload`, `_normalize_task_family_scores`, `_normalize_unsupported_task_types`, `_routes_from_registry_payload`). (3) `_BUILTIN_ROUTE_FALLBACKS` still lives in `router.py` despite the registry being owned by `route_registry.py`. | Original item (1) route metadata guard allowlist drift was resolved in LTO-9 Step 1. Remaining items (2)/(3) are hygiene-only and can be picked up on touched-surface basis. | When next change touches `router.py` / `route_policy.py` / `route_registry.py` |
| Surface / CLI / Meta Optimizer Split (LTO-9) follow-up | LTO-9 Step 1 intentionally deferred broad task / knowledge / artifact CLI command-family migration and the remaining route command adapters (`route registry show/apply`, `route policy show/apply`, `route select`). | Non-blocking review concern; treat LTO-9 as "Step 1 done", not fully complete. Continue on a future touched-surface / LTO-9 step rather than reopening this PR. | Post-merge roadmap update and next surface command-family phase |
