...
| 49 | S3: Librarian Agent Entity | `cli.py` 的 `stage-promote` 命令使用 `LIBRARIAN_MEMORY_AUTHORITY`（`”canonical-promotion”`）作为 `caller_authority`，与 `LibrarianAgent` 自动 promote 使用同一 authority 标识，审计时无法区分 CLI operator 手动操作与 Agent 自动操作 | Phase 50 将 CLI operator promote 路径切换为 `OPERATOR_CANONICAL_WRITE_AUTHORITY` |
...
| 51 | S2: Meta-Optimizer 独立 Agent 生命周期 | `MetaOptimizerAgent.memory_authority = "canonical-write-forbidden"` 容易被误解为“完全只读、无任何 side effect”，但 Agent 按设计仍会写 proposal/report artifact。语义实际禁止的是 canonical store 写入，不是所有文件写入 | 后续 phase 收紧 taxonomy / authority 命名或文档说明时，明确区分 canonical write authority 与 artifact write side effect |
| 52 | Closeout / review follow-up | `codex_fim` 仍是稳定 dialect key，且 `dialect_adapters/codex_fim.py`、部分历史 telemetry / fixture 继续保留 codex 命名语义。执行器品牌已切到 aider/claude-code，但方言标识未在本轮同步去品牌化 | 后续 phase 若要继续收紧命名语义，再统一评估 dialect key、文件名与 fixture 兼容迁移 |
