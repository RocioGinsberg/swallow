...
最近一次 triage：2026-05-04(post-v1.8.0 + LTO-1 closeout)。结论:LTO-9 Step 2 / LTO-1 review fixes 落地后,Active Open 中两项已可移到 Resolved;LTO-13 review C1 / N1 + LTO-1 review C1 三处 follow-up 已在各自 phase 内消化,记入 Resolved。无新 blocker。Active Open 现仅保留 4 项:R-observation quality / LTO-6 facade alias / LTO-7 私名耦合 / LTO-8 debate_loop_core callable。前两轮 triage:2026-05-01(R-entry / backlog hygiene)。
...
| Knowledge Plane facade naming cleanup (LTO-6) | LTO-6 PR review C1: paired `render_*` / `build_*` aliases in `knowledge_plane.py` inflate the public facade surface after caller migration has completed (~14 处 still active in `knowledge_plane.py:178-507`). | Non-blocking; LTO-1 第一阶段实现未顺手清理。仍建议下一次触动 `knowledge_plane.py` 时一并删除冗余别名,每对保留 `render_*` 用于报告渲染 / `build_*` 用于对象构造。 | Next phase that touches `knowledge_plane.py`(Wiki Compiler 第二阶段、LTO-2 retrieval quality follow-up 等)|
| Provider Router Split (LTO-7) follow-up | (2) `router.py` reaches into `_`-prefixed names in `route_policy.py` / `route_registry.py` (`_apply_route_policy_payload`, `_normalize_task_family_scores`, `_normalize_unsupported_task_types`, `_routes_from_registry_payload`). (3) `_BUILTIN_ROUTE_FALLBACKS` still lives in `router.py` despite the registry being owned by `route_registry.py`. | Original item (1) route metadata guard allowlist drift was resolved in LTO-9 Step 1. Remaining items (2)/(3) are hygiene-only and can be picked up on touched-surface basis;**截至 2026-05-04 仍 active**(`provider_router/router.py:52-156` 仍引用以 `_` 开头的内部名)。 | When next change touches `router.py` / `route_policy.py` / `route_registry.py` |
