...
| 34 | S2: CodexFIMDialect | `CodexFIMDialect.format_prompt()` 未转义任务标题/目标中的 `<fim_prefix>` / `<fim_suffix>` 字符串，可能扰乱 FIM prompt 结构 | 当引入外部用户输入 task 创建路径或统一 prompt escaping 时 |
