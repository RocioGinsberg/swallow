...
最近一次 triage：2026-04-30（v1.5.0 / R-entry gate）。结论：当前 Open 项没有必须在进入真实使用测试前修复的 blocker。Release preflight 中暴露的 subtask timeout wall-clock flaky test 已在 tag-prep 修正；剩余 Open 项按 R 阶段观察项或后续设计项处理，不阻塞已发布 `v1.5.0` 后的实际使用测试。
...
| 后续设计债 | Phase 66 design-needed bundle、event backfill、guard strength、audit payload size policy、runtime defaults / taxonomy constants 等 | 继续留作 roadmap / concerns backlog，不在实际使用测试前抢修 |
...
| 66 | design-needed: table-driven CLI dispatch | Phase 66 Block 5 found no dead CLI subcommand, but `cli.py` maintains parallel parser registration and dispatch chains with 80+ parser entries. Phase 67 M3 partially consumed the read-only artifact/report printer subset into `ARTIFACT_PRINTER_DISPATCH`; parser registration, governance write commands, and `task inspect` / `task review` remain explicit | Partial; future CLI cleanup phase can evaluate broader parser/dispatch alignment after the read-only subset has settled |
...
| 66 | design-needed: artifact-name registry / owner table | Phase 66 audit found task artifact names repeated across orchestrator, harness, retrieval allow-lists, and CLI artifact printers. Phase 67 M2 chose the narrow option: no global registry; M3 may partially consume CLI read-only printer ownership only | Follow-up design phase after M3; decide which artifacts are stable public surface, retrievable, or intentionally private |
