...
最近一次 triage：2026-04-26（Phase 60 启动前）。结论：当前 Open 项没有必须并入 Phase 60 的 blocker；retrieval-adjacent concern（sqlite-vec WARN 竞态、embedding dimensions import 固化）仍保留，但 Phase 60 只做 retrieval request source policy，不顺手修改 retrieval adapter / embedding 配置层。
