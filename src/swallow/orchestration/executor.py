...
def _load_fallback_route(
    ...
    from swallow.provider_router.router import lookup_route_by_name
    ...
    with tempfile.TemporaryDirectory(prefix="swallow-detached-") as tmp:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
                "swallow.orchestration.executor",
                "--detached-child",
                "--state-json",
                str(state_json),
                "--retrieval-json",
...
def run_local_executor(
    ...
    output = "\n".join(
        [
            ...
            "- answer_contract: run_record_not_semantic_qa",
            ...
            "",
            "## Boundary",
            "This output is a local execution summary and retrieval-guided next action, not a semantic answer to the task goal.",
            "",
            "## Risks",
            risks,
            "",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="python -m swallow.orchestration.executor")
