...
同时，当前系统也不应被理解为一套固定品牌编队。更贴近当前实践的公开表述是：**架构上坚持 role-first，而默认工作组合以 Claude Code 处理高价值、高复杂度任务，Aider 处理高频实现，Warp / Oz 处理终端原生并行 worker 任务。**

同时，当前知识架构不应理解为“先向量检索，再组织知识”，而应理解为：**先以 SQLite-backed truth objects 归一知识，再通过精确检索、策略过滤、向量召回与文本回退提供 retrieval service**。
